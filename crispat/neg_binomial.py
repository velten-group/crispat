...
def fit_NegBinomial(gRNA, adata_crispr, batch_list, output_dir, seed, n_iter, subsample_size):
    '''
...
def ga_negative_binomial(input_file, output_dir, start_gRNA = 0, gRNA_step = None, batch_list = None, UMI_threshold = 0,
                         n_iter = 2500, subsample_size = 15000, parallelize = True, n_processes = None, mem_limit = '10GB'):
    '''
    Guide assignment with a negative binomial mixture model based on SCEPTRE mixture approach 
    (Negative Binomial instead of Poisson as in SCEPTRE)
    ...
    # Fit NegBinomial model in parallel to all gRNAs
    if parallelize:  
        if n_processes == None:
    ...
    # Fit NegBinomial model without parallelization
    else:
        for gRNA in tqdm(gRNA_list):
            time.sleep(0.01)
            # fit the model to infer the parameters
            loss, map_estimates = fit_NegBinomial(gRNA, adata_crispr, batch_list, output_dir, 2024, n_iter, subsample_size)
            losses = pd.concat([losses, pd.DataFrame({'gRNA': [gRNA], 'loss': [loss]})])
            estimates = pd.concat([estimates, map_estimates])

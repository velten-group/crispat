...
def get_perturbed_cells(adata_crispr, estimates, gRNA):
    ...
    data['p0'] = binom.pmf(data['UMI_counts'], data['seq_depth'], data['prob0']) * (1 - pi)
    data['p1'] = binom.pmf(data['UMI_counts'], data['seq_depth'], data['prob1']) * pi
...
def ga_binomial(input_file, output_dir, start_gRNA = 0, gRNA_step = None, batch_list = None, 
                n_iter = 500, subsample_size = 15000, parallelize = True, n_processes = None):
    '''
    ...
    if not os.path.exists(output_dir + "loss_plots/"):
    ...
    # Fit Binomial model without parallelization
    else:
